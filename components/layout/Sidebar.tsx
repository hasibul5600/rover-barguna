 ...
 * below xl, so without this there is no way to move around on a phone.
 */
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className="grid size-10 shrink-0 place-items-center rounded-xl border border-emerald-950/10 text-[color:var(--forest)] transition hover:bg-emerald-950/5 xl:hidden"
      >
      ...
      {/* Portalled to <body>: the header carries backdrop-blur, which makes it a
          containing block for fixed children, so rendering the drawer in place
          collapsed it to the header's 80px height. */}
